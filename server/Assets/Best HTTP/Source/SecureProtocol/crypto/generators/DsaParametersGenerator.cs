...
// 3. n = ceiling(L 鈦冞outlen) 鈥掃1.
            int n = (L - 1) / outlen;

// 4. b = L 鈥掃1 鈥掃(n 鈭栠outlen).
            int b = (L - 1) % outlen;
...
// 11. For counter = 0 to (4L 鈥掃1) do
                int counterLimit = 4 * L;
                for (int counter = 0; counter < counterLimit; ++counter)
                {
// 11.1 For j = 0 to n do
//      Vj = Hash ((domain_parameter_seed + offset + j) mod 2^seedlen).
// 11.2 W = V0 + (V1 鈭栠2^outlen) + ... + (V^(n鈥掟) 鈭栠2^((n鈥掟) 鈭栠outlen)) + ((Vn mod 2^b) 鈭栠2^(n 鈭栠outlen)).
...
// 11.3 X = W + 2^(L鈥掟). Comment: 0 鈮＿W < 2L鈥掟; hence, 2L鈥掟 鈮＿X < 2L.
                    BigInteger X = W.Add(BigInteger.One.ShiftLeft(L - 1));

// 11.4 c = X mod 2q.
                    BigInteger c = X.Mod(q.ShiftLeft(1));

// 11.5 p = X - (c - 1). Comment: p 鈮犨1 (mod 2q).
                    BigInteger p = X.Subtract(c.Subtract(BigInteger.One));
